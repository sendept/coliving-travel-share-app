...
export const spanishPatterns: PatternSet = {
  ...
  from: /(?:desde|de|saliendo\s+de)\s+([^,]+?)(?=\s+(?:a|hasta|hacia|para|y|$))/i,
...
export const englishPatterns: PatternSet = {
  ...
  from: /(?:from|departing\s+from|leaving\s+from)\s+([^,]+?)(?=\s+(?:to|towards|and|$))/i,
...
export const frenchPatterns: PatternSet = {
  ...
  from: /(?:de|depuis|partant\s+de)\s+([^,]+?)(?=\s+(?:à|vers|jusqu'à|et|$))/i,
  to: /(?:à|vers|jusqu'à)\s+([^,]+?)(?=\s+(?:de|depuis|via|par|et|$))/i,
  via: /(?:via|par|en\s+passant\s+par|avec\s+arrêt\s+à)\s+([^,]+)/i,
